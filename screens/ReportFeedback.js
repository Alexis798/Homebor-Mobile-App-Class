...
export default class ReportFeedback extends Component {
    ...
    ImagesCache = () => {
        this.state.info[0].reportslist.map(async (item) => {
            ...
            if(item.report_img != 'NULL') {
                const photoReport = `YOURENDPOINT/${item.report_img}`;
  ...
  onRefresh = () => {
    ...
    registerfile1 = async () => {
        ...
        return await fetch(`YOURENDPOINT?des=${des}&email=${eMail}&idnoti=${idnoti}&name_h=${name_h}&l_name_h=${l_name_h}&a_name=${a_name}&a_mail=${a_mail}&stu_rep=${stu_rep}&status=${status}&photo1=${photo1}`, {
            method: 'POST',
            body: formData,
            header: {
