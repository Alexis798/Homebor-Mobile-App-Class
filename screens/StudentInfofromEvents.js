							...
							renderItem={({item}) => (
								<View>
									...
									<View style={ globalStyles.profileMargins}>
										{(Dimensions.get('window').width >= 414 && (Platform.isPad === true || Platform.OS === 'android')) && (
											<Center mt="-10%">
												<Avatar size="lg" bg="#232159" style={globalStyles.profileStudent} source={ item.photo_s != "NULL" && { uri: `YOURENDPOINT/${item.photo_s}` }}>{item.photo_s.toUpperCase().charAt(0)}</Avatar>
											</Center>
										)}
										{(Dimensions.get('window').width < 414 || (Platform.isPad != true && Platform.OS != 'android')) && (
											<Center mt="-15%">
