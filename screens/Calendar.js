  ...
  constructor(props){
    ...
    ImagesCache = async() => {
      ...
      if(this.state.info[0].phome != 'NULL') {
        const photoHome = `YOURENDPOINT/${this.state.info[0].phome}`;
    ...
    tokenProccess = async () => {      
      ...
      return await fetch(`YOURENDPOINT/tokenvallapp.php?email=${email}&token=${tokenval}`, {
        method: 'POST',
        header: {
    ...
    registerToken = async () => { 
      ...
      return await fetch(`YOURENDPOINT/tokenapp.php?email=${email}&token=${tokenval}`, {
          method: 'POST',
          header: {
            'Content-Type': 'multipart/form-data'
          },
