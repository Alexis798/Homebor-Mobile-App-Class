...
export default class CrearCuenta extends Component{
	...
	register = async () => {
		if (this.state.name == '' || this.state.lastname == '' || this.state.email == '' || this.state.password == '' || this.state.id_m == 'NULL' || this.state.itemTerms == false){
			...
			return await fetch(`YOURENDPOINT/validationusersapp.php?email=${email}&password=${password}`, {
					method: 'POST',
					header: {
	...
	register2 = async () => {
		...
		return await fetch(`YOURENDPOINT/registerApp.php?name=${name}&lastname=${lastname}&email=${email}&password=${password}&id_m=${id_m}`, {
				method: 'POST',
				header: {
					'Content-Type': 'multipart/form-data'
				},
							...
							<Center style={Platform.OS === 'ios' ? globalStyles.pickerviewCrearCuenta :  this.state.requiredFields == true && this.state.id_m == 'NULL' ? globalStyles.pickerAndroidError : globalStyles.pickerAndroid}>
								...
								<Picker
									...
									onValueChange={(id_m) => this.setState({id_m})}>
										...
										<Picker.Item color={this.state.requiredFields == true && this.state.id_m == 'NULL' ? '#DC2626' : 'black'} label="EMPRESS" value="10" />
						...
						<FormControl isInvalid={this.state.requiredFields == true && this.state.itemTerms == false && true}>
							<View style={globalStyles.editSelectsSquare}>
								...
								<Text style={globalStyles.labelSelectEdit}>I Agree With <Text style={globalStyles.labelSelectEditTermsConditions}
									onPress={() => Linking.openURL('YOUR END POINT')}>
