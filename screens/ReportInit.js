...
export default class Reports extends Component { 
    ...
    ImagesCache = () => {
        this.state.info != undefined && this.state.info[0].studentslist != undefined && this.state.info[0].studentslist.map(async (item) => {
            ...
            if(item.photo != 'NULL') {
                const photoStudents = `YOURENDPOINT/${item.photo}`;
    ...
    modalnotify = async () => {
        ...
        return await fetch(`YOURENDPOINT?email=${email}&mail=${mail}`, {
                method: 'POST',
                header: {
    ...
    registerfile1 = async () => {
        ...
        formData.append('photo', { uri: localUri, name: filename, type: type });
          ...
          return await fetch(`YOURENDPOINT?name_h=${name_h}&l_name_h=${l_name_h}&email=${eMail}&managermail=${managermail}&agency=${agency}&mail=${mail}&des=${des}&idnoti=${idnoti}&report=${report}&photo1=${photo1}`, {
            method: 'POST',
            body: formData,
            header: {
                'Content-Type': 'multipart/form-data'
